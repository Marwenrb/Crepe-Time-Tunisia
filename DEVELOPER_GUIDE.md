...
Crêpe Time Tunisia repose sur une architecture **MERN** moderne — MongoDB, Express, React, Node.js — enrichie de TypeScript côté serveur et d'un frontend Vite ultra-rapide. Ce socle technique a été conçu pour offrir une expérience fluide aux clients de Nabeul tout en restant extensible vers de nouveaux points de vente.

```
┌─────────────────────────┐          ┌─────────────────────────┐          ┌────────────────┐
│   React + Vite (5173)   │   HTTP   │  Express + TS (5000)    │  Mongoose│   MongoDB      │
│                         │─────────▶│                         │─────────▶│                │
│  Tailwind CSS           │          │  JWT Auth               │          │  - Users       │
│  Shadcn/ui              │          │  Helmet + CORS          │          │  - Orders      │
...
Les prix sont stockés en **millimes** (centièmes de dinar tunisien) dans MongoDB afin d'éliminer toute erreur de virgule flottante. Un prix affiché de `10.00 TND` correspond à la valeur `1000` en base de données.
...
4. L'URL publique CDN retournée par Cloudinary est sauvegardée dans le document MongoDB
...
**MongoDB Atlas :** Utilisez un cluster M0 (gratuit) ou supérieur. Assurez-vous que l'IP du serveur backend est autorisée dans *Network Access*.
...
Pour des volumes importants, activez le sharding sur la collection `orders` avec un shard key sur `restaurant` + `createdAt`. Cette stratégie distribue la charge de manière optimale entre les noeuds.
...
Introduisez Redis pour mettre en cache les résultats de recherche et les menus fréquemment consultés. Cela réduit la charge sur MongoDB et permet des temps de réponse sous les 50 ms.
...
- Vérifier que MongoDB est accessible et que `MONGODB_URI` est correcte dans `.env`
